...
canonical-phrases.mdファイルは、AI実行精度を最大化するための標準化された用語、曖昧な用語の具体的基準、明確な決定木を定義しています。
