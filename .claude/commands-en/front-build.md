...
**CRITICAL**: Parse every sub-agent response for its routing meaning. Proceed to the next task only after quality-fixer-frontend returns `approved`.
...
3. **Fix cycle** (when any verifier failed):
   ...
   - After correction review converges, run quality-fixer-frontend once. Continue after `approved`; present the exact user-owned decision if it returns `blocked`.
