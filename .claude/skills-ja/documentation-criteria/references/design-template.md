...
本セクションは**この設計が**現在の設計サーフェスから除外した機能・能力を記録する。エントリは現要件、現在のコンシューマー、または記録された制約に結び付くものに限定し、純粋な将来構想は別途の提案ドキュメントに送る。

除外を記録するセクションは3つあり、誰がどの粒度で決めたかで分かれる:

| セクション | 記録する内容 | 決めた主体 |
|---|---|---|
| `Requirement Convergence` | 非ゴールと投機的要件 | ユーザーが、要件の段階で |
| `Design Convergence → Rejected Additions` | 特定の Failed Item を解決する過程で検討した選択肢 | この設計が、Failed Item に結び付けて |
| 本セクション | 個別の Failed Item に結び付かない能力レベルの除外 | この設計が、能力の粒度で |
