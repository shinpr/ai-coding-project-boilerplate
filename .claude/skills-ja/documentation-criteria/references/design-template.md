...
### エラー状態マトリクス
| コンポーネント | エラー種別 | 表示 | 回復アクション |
|-------------|----------|------|-------------|
| [コンポーネント名] | [ネットワーク/バリデーション/権限] | [エラーメッセージ/フォールバックUI] | [リトライ/リダイレクト/問い合わせ] |

### エラーバウンダリ戦略
- グローバルエラーバウンダリ: [配置場所と表示内容]
- 機能別エラーバウンダリ: [スコープと粒度]
- フォールバック優先順位: [ユーザーデータ保持 > 部分表示 > エラーページ]
...
| 状態カテゴリ | 管理方法 | 永続化 | 例 |
|------------|---------|-------|-----|
| UIローカル状態 | useState | なし | モーダル開閉、フォーム入力中 |
| 機能横断状態 | Context/Store | なし | 認証状態、テーマ |
| サーバー状態 | React Query/SWR | キャッシュ | APIレスポンス |
| URL状態 | Router | URL | ページ、フィルタ条件 |

### キャッシュ戦略
- stale time: [期間と根拠]
- キャッシュ無効化トリガー: [ミューテーション後の自動 / 手動]

## UIアクション - APIコントラクトマッピング（フロントエンド機能を含む場合）

| UIアクション | APIエンドポイント | リクエスト型 | レスポンス型 | エラーハンドリング |
|------------|----------------|-----------|-----------|----------------|
| [ボタンクリック等] | [POST /api/xxx] | [RequestDTO] | [ResponseDTO] | [エラー表示方法] |
