...
変更する責務を統制する承認済みADRを、承認済みの共通ADRも含めて列挙し、この設計が記録された各判断と整合することを確認する。
...
- [観測されたパターン] `[implicit]` — 根拠: [ファイルパス] — 確認: [済/未]
...
検証前の未解決の前提は `確認: 未` とし、必要なエビデンスと設計への影響を記す。Selected Design を左右する前提は承認前に解決する。検証後に `確認: 未` を残せるのは、実装上の不確実性のうち、どの結果でも Selected Design が有効なものだけである。その場合は対応する「リスクと対策」の行で同じ主張をリスクとして再掲し、検証またはガードの方法と `検証先: [ステップまたは成果物]` を記す。
...
レビューを起点とする新規のupdateでcapability probeを実行した場合にのみ記載する。ドキュメントに残す記録は簡潔にし、生ログと一時成果物はprobeのクリーンアップ時に破棄する。

| apply した finding | 前提 | 手法と観測境界 | 観測結果 | 制約 | 設計への影響 |
|---|---|---|---|---|---|
| [finding ID] | [正確な前提] | [範囲を限定した手法と、利用側から観測できる事後条件] | [観測結果] | [残る制約と、それによって限定される設計への影響。なければ None] | [Selected Design の変更または確認] |

#### 品質保証メカニズム
変更対象領域で品質がどのように担保されているか。各項目は adopted（実装時に適用）または noted（観測されたが不採用、理由付き）のいずれか。
...
渡されたコード分析またはUI分析の`focusAreas`の各エントリに対して1行ずつ記載する。この表は**構造的な既存事実**を設計に結び付ける。既存の振る舞いに言及する他セクションは、この表の行を`fact_id`値で参照する。
...
ユーザーの判断、設定、モード、概念、出力、永続状態、実装経路、公開コントラクト、抽象、サービス、コンポーネント分割のうち、新たに追加する設計要素ごとに次を記録する:

- **追加**: [選択した設計要素]
- **現在のエビデンス**: [要件、承認済みの決定、検証済みの制約、観測された問題、根拠のある重大リスク]
- **設計範囲をこれ以上小さくできない理由**: [再利用、導出、オンデマンド計算、現在の境界での責務保持では同じ条件を満たせない理由]
- **取り除いた場合の結果**: [取り除くと満たせなくなる、確認済みの成果、必要な境界、または証明]

選択した設計でこれらの要素を追加しない場合は、`None — 既存の設計で十分` と記載する。
...
UI Specで表示すると定めた値は、その画面やコンポーネントに値を渡す契約にも含める。指定された表示を実現できないフィールド構成は、簡略化ではなく不完全な契約である。
...
- **成功基準**: [観測可能な成果 — 例: 「CSVダウンロードが既存と同一の出力を生成」「APIが期待スキーマで200を返す」]
...
現行要件、実装、または検証に必要な境界を記録する。ここに置くのは選択した設計の境界であり、調査で見つけた候補や未選択の将来案ではない。
...
Selected Design を有効なまま保ちつつ、rollout・rollback・契約の扱い・検証戦略を変えうる、エビデンスのある残存リスクだけを記載する。
