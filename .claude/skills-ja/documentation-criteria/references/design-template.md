...
確認済みの要件と境界を記録する。評価依頼、推測的なアイデア、未選択の実装手段は要件確認前の収束コンテキストに留める。PRDがこれらを保持する場合は前半2項目を`N/A — PRD [パス] がカバー`とし、リバースエンジニアリング／現状記述のドキュメントでは本セクション全体を`N/A — リバースエンジニアリング／現状記述ドキュメント`とする。未解決の論点はいずれの場合も本セクションに残す — 設計に着手できるかがそれに依存するためである。
...
選択したエンドツーエンドの経路全体を記述する。成果物に残すのは選択した経路だけとし、比較した選択肢を判断の履歴として残せるのは承認済みADRが扱う場合に限る。リバースエンジニアリング／現状記述のドキュメントでは、本セクション全体を `N/A — リバースエンジニアリング／現状記述ドキュメント` とする。
