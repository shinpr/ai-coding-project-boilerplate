...
**ユーザーが**要件の段階で決めた除外を記録する。この設計が決めた除外は`将来の拡張性`に記載する。PRDがこれらを保持する場合は前半3項目を`N/A — PRD [パス] がカバー`とし、リバースエンジニアリング／現状記述のドキュメントでは本セクション全体を`N/A — リバースエンジニアリング／現状記述ドキュメント`とする。未解決の論点はいずれの場合も本セクションに残す — 設計に着手できるかがそれに依存するためである。

- **Outcome**: [この変更が生むべき観測可能な結果を1つ]
- **Non-Goals**: [ユーザーが除外した機能・能力 | None — 除外すべきものはないとユーザーが確認]
- **Speculative**: [ユーザーが口にしたが決めていない案 -> 先送りの理由 | None]
...
将来状態のドキュメントでは、`Failed Items`、`Adopted Additions`、`Rejected Additions` が空の場合に `None` と記載する。リバースエンジニアリング／現状記述のドキュメントでは、本セクション全体を `N/A — リバースエンジニアリング／現状記述ドキュメント` とする。

1. **Direct MVP**: [既存のシステム機能を使った最も単純なエンドツーエンドの設計]
2. **Failed Items**: [満たせない項目 → 要件、検証済みの制約、観測された問題、根拠のある重大リスク | None]
3. **Adopted Additions**: [追加 → 対応する Failed Item → より小さいサーフェスでの解決が成立しない根拠 → 取り除くと当該項目がその根拠に照らして再び満たせなくなること | None]
4. **Rejected Additions**: [検討した選択肢 → 簡潔な不採用理由 | None]
...
本セクションは**この設計が**現在の設計サーフェスから除外した機能・能力を記録する。ユーザーが要件の段階で決めた除外は `Requirement Convergence` に記載する。エントリは現要件、現在のコンシューマー、または記録された制約に結び付くものに限定し、純粋な将来構想は別途の提案ドキュメントに送る。

- **先送りした機能・能力**: [設計時に検討したが現在の設計サーフェスから明示的に除外した機能・能力。各エントリには、これが満たしたはずの現要件を明記する]
