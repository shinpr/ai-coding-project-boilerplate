...
実装がパッケージ、サービス、またはプロセス境界をまたがる場合、**または値がシリアライズされ単一ランタイム内であっても境界をまたいで再パースされる場合**（クエリ文字列、ルート/CLI引数、環境変数、設定エントリ、メッセージ/キューのペイロード、ストレージキー、ファイルなどの媒体を介する。producerとconsumerは正確な表現をあらかじめ取り決めておく必要がある）に本セクションを記載する。各境界を文書化することで、task-decomposerは境界の文脈を両側の実装タスクに伝播できる。各オーナーは、task-decomposerがexecutorの読めるInvestigation Targetとして追加できるよう、モジュール/パッケージ/コンポーネント名ではなく具体的なファイルパスで記録する。該当する境界がない場合は本セクションを省略する。
