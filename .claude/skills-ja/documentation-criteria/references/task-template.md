...
## Proof Obligations
（このタスクが実装するACまたはクレームごとに1エントリ。スケルトンの注釈がある場合はそこから、なければACの主要な故障モードから導出する。各テストはクレームを証明すること。下記ブロックをクレームごとに繰り返し、見出しにAC IDまたはクレームIDを記載して下流のレビューがクレームごとにカバレッジを解決できるようにする。）

### Obligation: [AC IDまたはクレームID]
- **クレーム**: [ACが約束する振る舞い]
- **主要な故障モード**: [テストがレッドになるリグレッション]
- **検証する境界**: [テストが通過する公開/統合境界、または "in-process unit"]
- **状態アサーション**: [状態変更を伴うクレームでは 操作前 → 操作 → 操作後 の観察可能な状態。それ以外は "N/A"]
