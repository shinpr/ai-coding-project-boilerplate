# [機能名] UI仕様書

## 概要

[このUI仕様書の目的とスコープを2-3文で説明]

### 確認済み要件のコンテキスト
- PRDパス: [docs/prd/xxx-prd.md | "N/A — 要件分析の出力に基づく"]
- 機能スコープ: [このUI Specがカバーする確認済み要件]

### 設計ソース
| ソース | パス | バージョン |
|--------|------|-----------|
| プロトタイプコード | [docs/ui-spec/assets/xxx/] | [コミットSHA / タグ] |

## プロトタイプ管理

プロトタイプコードはこのUI Specの**添付資料**である。このドキュメントとDesign Docを正規の仕様とし、差異がある場合は両ドキュメントを優先する。

- **添付パス**: [docs/ui-spec/assets/{feature-name}/]
- **バージョン識別**: [コミットSHA / タグ]
- **準拠前提**: [例: デザインシステム準拠、コンポーネントライブラリ使用]
- **参照レベル**: [`binding` — このドキュメントで別の内容を定めた箇所以外は添付資料の表示に従って実装する | `reference` — このドキュメントに記録した内容だけを実装へ渡す]
- **正式な仕様との関係**: どちらの参照レベルでも、プロトタイプとこの仕様に差異がある場合は、このドキュメントを優先する

## ACトレーサビリティ（プロトタイプ）

確認済みの受入条件をプロトタイプの参照先にマッピングし、既存のAC IDがある場合は保持する。プロトタイプが提供されていない場合はこのセクションを省略。

| 条件ID / 参照 | 条件概要 | 画面 / 状態 | プロトタイプ参照先（要素ID / パス） | 採用判定 |
|-------|--------|------------|----------------------------------|---------|
| AC-001 | [EARS AC概要] | [画面名 / 状態名] | [要素またはファイルの参照先] | 採用 / 不採用 / 保留 |

## 画面リストと遷移

重要なインタラクションまたは階層が下表だけでは明確にならない場合に限り、Mermaidによる画面遷移図またはコンポーネントツリー図を追加する。

### 画面リスト

| 画面ID | 画面名 | 説明 | 到達条件 |
|--------|-------|------|---------|
| S-01 | [画面名] | [目的] | [ユーザーがこの画面に到達する方法] |

### 遷移条件

| 遷移元 | 遷移先 | トリガー | ガード条件 |
|--------|-------|---------|-----------|
| S-01 | S-02 | [ユーザーアクション] | [前提条件（ある場合）] |

## コンポーネント分解

### コンポーネントツリー

```
[ページ/画面]
  +-- [コンテナコンポーネント]
  |   +-- [プレゼンテーショナルコンポーネントA]
  |   +-- [プレゼンテーショナルコンポーネントB]
  +-- [コンテナコンポーネント]
      +-- [プレゼンテーショナルコンポーネントC]
```

### コンポーネント: [ComponentName]

> コンポーネント見出しの一意性: すべての`コンポーネント: [ComponentName]`見出しはUI Spec内で一意でなければならない。下流の工程は見出しテキストの完全一致でコンポーネントを参照するため、重複した名前や言い換えがあると実装タスクへの伝播が破綻する。

#### 状態×表示マトリクス

本マトリクスには、確認済み要件・承認済みのUI方針・維持される振る舞い・リポジトリ/デザインシステムのルールに裏付けられた状態を1行ずつ記載する。

「表示」欄には描画結果を記載し、データや文言を含む場合は値の出所も記載する。プロトタイプがある場合は、確認済みの成果のために調査した範囲内で、データや文言を持つ要素のうち採用するものをすべてこの欄で扱う。データや文言を持つ要素のうち採用しないものは、その理由とともに該当するコンポーネントの表の直下へ記載する。サイズ、余白、色、アイコンの形状などの見た目の属性は、記録した参照レベルでまとめて扱う。これらは要件が変わらなくても変化し、列挙するとドキュメントがすぐ古くなるためである。

| 状態 | トリガー | 表示 | 操作 / 回復 | 根拠 |
|------|---------|-----|------------|-----|
| [該当する状態] | [その状態を発生させる条件] | [描画結果（データや文言を含む場合は値の出所も記載）] | [可能な操作、または N/A] | [要件・UIのエビデンス・リポジトリのルール] |

#### インタラクション定義

| 条件ID / 参照 | EARS条件 | ユーザーアクション | システムレスポンス | 状態遷移 | エラーハンドリング |
|-------|---------|-----------------|------------------|---------|-----------------|
| AC-001 | When [トリガー] | [クリック / 入力 / 等] | [期待される動作] | [遷移元状態 → 遷移先状態] | [リトライ / リセット / フォールバック] |

### コンポーネント: [ComponentName2]

[スコープ内の各インタラクティブコンポーネントについて、該当する状態×表示マトリクスとインタラクション定義を繰り返す]

## デザイントークンとコンポーネントマップ

### 環境制約

- 対象ブラウザ: [例: Chrome 120+, Safari 17+]
- テーマ対応: [例: ライト/ダーク、システム設定に追従]

#### レスポンシブ動作

| ブレークポイント | 幅 | 主な変更点 |
|----------------|-----|-----------|
| モバイル | [例: < 768px] | [例: シングルカラム、ハンバーガーナビ、本文14px] |
| タブレット | [例: 768px - 1023px] | [例: 2カラムグリッド、サイドバー折りたたみ] |
| デスクトップ | [例: ≥ 1024px] | [例: フルレイアウト、ナビ展開、サイドバー表示] |

### 既存コンポーネント再利用マップ

| UI要素 | 判定 | 既存コンポーネント | 備考 |
|--------|------|------------------|------|
| [ボタン] | 再利用 | [components/ui/Button] | [修正不要] |
| [データテーブル] | 拡張 | [components/ui/Table] | [ソート機能を追加] |
| [機能カード] | 新規 | - | [類似コンポーネントなし] |

### デザイントークン

#### Color Roles

| ロール | トークン | 値 | 用途 |
|--------|---------|-----|------|
| Background Surface | [bg-primary] | [例: #FFFFFF] | [ページ背景] |
| Background Surface | [bg-secondary] | [例: #F9FAFB] | [カード、セクション背景] |
| Text | [text-primary] | [例: #111827] | [見出し、本文] |
| Text | [text-secondary] | [例: #6B7280] | [キャプション、プレースホルダー] |
| Brand / Accent | [color-brand] | [例: #1A73E8] | [プライマリアクション、リンク] |
| Status | [color-success] | [例: #22C55E] | [成功状態、確認] |
| Status | [color-error] | [例: #EF4444] | [エラー状態、破壊的アクション] |
| Border | [border-primary] | [例: #E5E7EB] | [カードボーダー、区切り線] |

#### Typography Hierarchy

| ロール | フォント | サイズ | ウェイト | 行間 | 字間 |
|--------|---------|--------|---------|------|------|
| Heading 1 | [例: Inter] | [例: 30px] | [例: 700] | [例: 1.2] | [例: -0.02em] |
| Heading 2 | [例: Inter] | [例: 24px] | [例: 600] | [例: 1.3] | [例: -0.01em] |
| Body | [例: Inter] | [例: 16px] | [例: 400] | [例: 1.5] | [例: 0] |
| Caption | [例: Inter] | [例: 12px] | [例: 400] | [例: 1.4] | [例: 0.01em] |
| Monospace | [例: JetBrains Mono] | [例: 14px] | [例: 400] | [例: 1.6] | [例: 0] |

#### Spacing Scale

| トークン | 値 | 用途 |
|---------|-----|------|
| [spacing-xs] | [例: 4px] | [インライン要素間のギャップ] |
| [spacing-sm] | [例: 8px] | [コンパクトなパディング] |
| [spacing-md] | [例: 16px] | [デフォルトのコンポーネントパディング] |
| [spacing-lg] | [例: 24px] | [セクション間隔] |
| [spacing-xl] | [例: 40px] | [ページセクション間の分離] |

#### Elevation（Depth）

| レベル | 値 | 用途 |
|--------|-----|------|
| 0 (Flat) | [例: none] | [インライン要素、テキスト] |
| 1 (Raised) | [例: 0 1px 2px rgba(0,0,0,0.05)] | [カード、ボタン] |
| 2 (Floating) | [例: 0 4px 12px rgba(0,0,0,0.1)] | [ドロップダウン、ポップオーバー] |
| 3 (Overlay) | [例: 0 8px 24px rgba(0,0,0,0.15)] | [モーダル、ダイアログ] |

#### Border Radius Scale

| トークン | 値 | 用途 |
|---------|-----|------|
| [radius-sm] | [例: 4px] | [バッジ、チップ] |
| [radius-md] | [例: 8px] | [カード、入力フィールド] |
| [radius-lg] | [例: 12px] | [モーダル、パネル] |
| [radius-full] | [例: 9999px] | [アバター、ピル] |

## ビジュアル受入条件

### ゴールデンステート
受入条件（AC）となる主要なビジュアル状態を定義:

1. **[状態名]**: [視覚的に確認すべき内容の説明]
2. **[状態名]**: [説明]

### レイアウト制約
- [最小/最大の幅・高さ制約]
- [コンポーネント間のスペーシングルール]
- [オーバーフロー時の挙動]

## アクセシビリティ要件

### キーボードナビゲーション

| コンポーネント | タブ順序 | キーバインド | 挙動 |
|-------------|---------|------------|------|
| [コンポーネント] | [順序番号] | [Enter / Space / Arrow] | [期待される動作] |

### スクリーンリーダー

| コンポーネント | ロール | アクセシブル名 | ライブリージョン |
|-------------|--------|-------------|---------------|
| [コンポーネント] | [ARIAロール] | [aria-label / aria-labelledby] | [polite / assertive / none] |

### コントラスト要件

| 要素 | 前景色 | 背景色 | 目標比率 |
|------|-------|-------|---------|
| [テキスト要素] | [色] | [色] | [通常テキスト4.5:1 / 大テキスト3:1] |

## 未確定事項

| ID | 説明 | ブロックする内容 | 必要な決定者 / 根拠 |
|----|------|----------------|-------------------|
| TBD-01 | [判断をブロックする未解決事項のみ] | [何が進められないか] | [誰が決めるか、または何があれば解決するか] |

ブロックしない不明点は本表から省くか、関連するセクション内に記載する。判断をブロックする項目は Design Doc 作成前に解決すること。

## 更新履歴

| 日付 | バージョン | 変更内容 | 作成者 |
|------|-----------|---------|-------|
| YYYY-MM-DD | 1.0 | 初版作成 | [名前] |
