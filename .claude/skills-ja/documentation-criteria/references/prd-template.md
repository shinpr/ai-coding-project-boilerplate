...
[散文では重要なユーザーフローが明確にならない場合に含める。不要なら本サブセクションごと削除する。]
```mermaid
journey
    title [機能名] ユーザージャーニー
    section [フェーズ1]
...
[散文では重要なスコープ内外の関係が明確にならない場合に含める。不要なら本サブセクションごと削除する。]
```mermaid
C4Context
...
上記の価値を届ける、最小のまとまった振る舞い・ジャーニー。ここに残るのは、取り除くと価値、または必要な法務・契約・セキュリティ・互換性の義務が成立しなくなる要件だけである。
...
ユーザーが明示した除外事項と、MVPの境界を実装判断に使える形で定めるために必要な現在の除外を記録する。評価依頼、推測的なアイデア、未選択の可能性は要件確認前の収束コンテキストに留める。

`Origin` は、ユーザーが明示した除外事項（`user`）と、確認済みのMVP境界を実装判断に使える形にするために必要な現在の除外（`analysis`）を区別する。ユーザーが除外を検討した結果そうしたものがなかった場合は `None — 除外すべきものはないとユーザーが確認` と記載する。
...
*各項目はユーザーと解決するか、ユーザーが未決のまま残すことに同意した項目として残す。空になったらこのセクションを削除*
