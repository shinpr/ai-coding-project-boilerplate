...
- **Design Doc** — 確認済みスコープに対する完全な実装設計として、責務、フロー、契約、変更影響、検証境界を記録する。実装では主要な技術基準として扱い、不足している実現方法を黙って作り出さない。リポジトリのエビデンスによって技術的な実現方法が成立しないと分かった一方で、確認済みの成果、将来状態の要件、対象外を維持できる場合は、プロダクト要件を開き直さず、担当するワークフローで実装と影響を受ける技術成果物を修正する。
