...
| 小規模（1〜2ファイルで、より高いリスク軸なし） | なし — 確定した成果、影響パス、検証条件を明示プロンプトでexecutorに渡す | 機能スコープ変更時は既存PRDを更新 | PRD更新（該当時）→ 実装 |
...
下記のADR作成条件のいずれかが該当する場合、確定規模はファイル数に関わらず**最低でも中規模**（Design Docと作業計画書が必須）とする。エスカレーションは規模を引き上げるだけで、ファイル数がすでに中規模・大規模に達している場合はそのまま据え置く。引き上げの根拠となった条件を、規模を決めた軸として記録する。
