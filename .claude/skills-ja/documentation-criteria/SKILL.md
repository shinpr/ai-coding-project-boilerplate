...
ファイル数は規模を測るが構造的な影響は測らないため、2ファイルの変更でも契約やデータフローを作り替えることがある。上記マトリクスの「リスク軸」はこのルールに解決される。
...
規模は、変更にどのドキュメントが必要かを決める。作業計画書内のタスク境界は別の判断であり、実装成果・ロールバック境界・Executor lane から決める（plan-template 参照）。
...
- 実装成果ごとに1つのタスクエントリ。各エントリは安定IDの`Phase X タスクY`、Target Files、ロールバック境界、Executor lane を持つ（plan-template.md 参照）
- それら安定IDで宣言するタスク依存関係（最大2階層まで）
- スケジュールと期間見積もり
