...
- **初動**: 他のステップより先に、初動アクション規則のハンドオフでrequirement-analyzerを呼び出す

### 初動アクション規則

新しいタスクを受け取ったら、ユーザーの発言全体はオーケストレーターが保持し、requirement-analyzerには調査対象だけを渡す。`requirements`には、課題、またはユーザーに見える成果や運用上の成果を述べたユーザーの文言のうち最も短いものをそのまま渡し、該当する文言がない場合に限り作業用の要約を使う。`context`には、その成果の解釈に必要な最小限のユーザーの理由、不可欠な場合に限り環境上の制約、そしてユーザーが名指ししたパス・識別子・成果物や、分析対象を変えたヒアリングの回答を渡す。オーケストレーターが選んだパスや技術的な質問は渡さない。返ってきたスコープ・コスト・質問のエビデンスを保持している文言と照らし合わせて要件収束を実行し、構造スケールを割り当てる。評価の依頼、推測的なアイデア、手段の指定は、保持している文言から分類する。アナライザに分類させると、推論がリポジトリの観察と同じ結果に混ざるためである。プロダクト要件と対象外はユーザーが持ち、収束の判定と構造スケールはオーケストレーターが持つ。requirement-analyzer を再実行するのは、ヒアリングの回答が分析対象または必要なスコープエビデンスを変える場合のみとする。

### フロー実行中の要件変更検知

分析結果が返ったら、その結果が示すユーザー向けまたは運用上の責務を、確認済みのスコープと照らし合わせる。その責務をそのまま残すこと、または変更することが確認済みの成果や対象外を変える場合は、依存する設計作業の前に要件ゲートへ戻す。確認済みのスコープをどう満たすかという選択は、設計の担当者に任せる。
...
レビュアーの検出事項は候補である。修正作業は、レビュー対応で`apply`となった集合からだけ作成する。レビュアーの`pass`はフェーズの通過を記録するものであり、ユーザーが持つ権限を与えず、レビューした手段が引き続き必要であることも示さない。
