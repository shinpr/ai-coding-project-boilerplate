...
現在の状態を再判定するverifierは、最新結果から少なくとも1件の修正を適用した後、または呼び出し元の再実行規則が現在の状態の結果を必要とする場合に再実行する。修正した項目については、最新結果がそれまでの判定結果を置き換える。最新結果が、正典のエビデンスが変わらないまま実質的に同じ主張または矛盾を報告した場合は、以前の`decline`を維持する。新しい、または実質的に変わった検出事項は、ルーティング前に対応方針を決める。実質的に同じ検出事項かどうかは、再生成された位置依存のIDだけではなく、主張や矛盾と、引用された出典・対象のエビデンスで照合する。
