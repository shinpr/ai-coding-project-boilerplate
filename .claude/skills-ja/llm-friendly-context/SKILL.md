...
実行中のワークフローが宣言した入力契約は、その担当スペシャリスト向けに既に最適化されている。名指しされたフィールドと宣言された値の形式は保持し、それらのフィールドと値からハンドオフを組み立てる。以下の一般的なプロンプト構成ルールは、入力契約が存在しない場合にのみ適用する。生成物向けのルールは生成物に適用する。
