...
ブラウザテスト基盤が存在しない場合は、integration-e2e-testingを使い、ブラウザレベルの証明が必要か判断する。必要な場合は、リポジトリと該当する外部エビデンスから必要十分で最も小さいテスト基盤を選ぶ。不要な場合は、より低コストで観測できる境界を使う。実行環境を利用できないことは、ユーザー判断ではなく証明不足として報告する。
