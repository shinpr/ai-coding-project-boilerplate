...
リポジトリのブラウザテスト設定、スクリプト、fixture、周辺のテスト、CIのルーティングを調べる。既存のテスト基盤、import、locatorの規約、setupのライフサイクル、ファイル名、テストの配置を維持する。

ブラウザテスト基盤が存在しない場合は、実ブラウザでなければ対象の振る舞いを観測できず、より低コストな既存の確認手段では証明できない場合に限り、ブラウザレベルの証明を必須とする。必要な場合は、リポジトリと該当する外部エビデンスから必要十分で最も小さいテスト基盤を選ぶ。それ以外は、より低コストで観測できる境界を使う。実行環境を利用できないことは、ユーザー判断ではなく証明不足として報告する。
...
- **永続化される結果** — 要件または契約が「リロードや再遷移をまたいで変更が残る」と規定している場合は、そこでも検証する。ページ内の更新は成功していても永続化が失敗しうるため。そうした契約のない一時的なstate — フィルタ選択、モーダルの開閉、ウィザードの進行 — はその場で検証する。ここでリロード後の検証を加えると、機能が約束していない保証をテストすることになる
...
**fixture-e2e の原則**:
- バックエンドはフェイクされ、稼働していない。これらのテストの実行にバックエンド起動スクリプトは不要
- フィクスチャはリポジトリ内（`tests/e2e/data/`）にバージョン管理される。マシンを越えて決定論的なテストになる
- ジャーニーに認証済み状態が必要だが、実際の認証を検証しない場合は、決定論的なテストCookieまたはfixture-mode sessionで認証状態を確立
...
不足する前提条件はテスト実装タスク自体の中で対処するか、テスト対象の振る舞いがライブ環境を必要としないのであれば検証を fixture-e2e へ移す。
...
4. `page.getByTestId()` — 最終手段。ユーザーから見える属性で要素を識別できない理由をテスト内に記録する

CSSのidやclassによる選択はユーザーから見えるものではなく、テストをマークアップに依存させるため、上記の一覧からLocatorを選ぶ。

## Viewportテスト

UI Specでレスポンシブ動作が定義されている場合、UI Specが挙げる各viewportで検証する。そのサイズで表示される要素と表示されない要素の両方を検証する — breakpointの不具合は両方の要素が同時にrenderされる形で現れることが多い。
