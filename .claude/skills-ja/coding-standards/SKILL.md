...
- **Design Convergence** - 現行の必要な成果を、新規の設計サーフェスが最小になる形で届ける。永続状態、公開コントラクトまたは境界を越える契約、振る舞いモード、再利用可能な抽象、コンポーネント分割を選ぶ際は、先に implementation-approach スキルの Phase 2（Design Convergence）を実行する
