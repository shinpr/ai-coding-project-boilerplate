...
- **Design Convergence** - 現行の必要な成果を、新規の設計サーフェスが最小になる形で届ける。永続状態、公開コントラクトまたは境界を越える契約、振る舞いモード、再利用可能な抽象、コンポーネント分割の選択は、それだけのサーフェスを伴うため、先に implementation-approach スキルの Phase 2（Design Convergence）を実行するに値する
...
- **IF** 外部依存（ライブラリ、プラグイン、SDK）を採用 → **THEN** 同じ依存のリポジトリ全体での使用分布を確認。マニフェスト、ロックファイル、既存の利用箇所から互換性のあるバージョンを特定できない場合はエスカレーション
