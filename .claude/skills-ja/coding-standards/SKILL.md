...
実装の前に、変更するコードの呼び出し元、依存関係、データフロー（生成→変更→参照）を、「変更境界と参照の代表性」で定める変更境界を別のファイルが変えなくなるまで追跡する。実装やその検証が依拠する直接・間接の影響は、その作業に引き継ぐ。

### 未使用コード削除ルール

未使用コードを検出したら、タスク完了までに、現行要件と到達可能な呼び出し経路から使用されているかを判断する。
