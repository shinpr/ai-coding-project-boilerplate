...
- **根拠で範囲を定めたリファクタリング** - 現在の成果を妨げるコード、今回のタスクで変更するコード、適用対象の品質チェックに失敗するコードを、振る舞いを保つ小さなステップで改善する
- **現行要件に必要なコードだけを追加** - 追加するすべてのコードパスは、現行要件または合意済みの技術的制約を満たすものとする（YAGNI）
