...
意図の保持、判断に必要な情報、情報密度、制約の必要性、追跡可能性を評価する。明記された指摘を解消する、または保持すべき要件を守る調整だけを適用する。
...
- 実行に不可欠なルールは`SKILL.md`に残すか、同一ディレクトリ内のreferenceを読み込む明示的なgated stepを設ける。各pure skillはsibling pure skillを読み込まなくても実行できる状態を保つ
