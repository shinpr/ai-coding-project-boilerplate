...
E2E候補とは、次のいずれかの性質を持つ振る舞いである。出典となる成果物のどこに記録されていてもよい。

| 性質 | E2E候補となる理由 |
|------|-----------------|
| 複数画面にまたがる | 約束された成果が、画面遷移の連なりを経て初めて観測可能になる |
| 実ブラウザを要する | ナビゲーション・Cookie・ストレージ・実DOM描画が結果を左右する |
| 遷移しないと到達できない状態 | プロセス内では再現できないエラー/Empty/ローディング状態 |
| ブラウザ依存のインタラクション | ドラッグ&ドロップ、キーボードナビゲーション、レスポンシブ動作 |

出典となる成果物を読み、これらの性質に対応付けるのはエージェントの役割である。本スキルは成果物のセクション名を持たない。
...
各候補を以下の形で記録し、レーン判定と上限適用のステップが消費できるようにする。
