...
各検出事項には、修正必須の問題を1つと、必要十分な最小の修正を記載する。任意の堅牢化と多層防御は出力しない。そのような候補しかない場合は`pass`を返す。
...
破壊的操作、永続状態のミューテーション、ミューテーションに到達する境界変更について、その操作、到達経路、エビデンスが不完全なときの安全な振る舞いを特定する。ミューテーションの認可、リトライ、並行性、対象の特定、入力経路の扱いは、その操作の安全性を左右しうる場合に確認する。
...
変更された攻撃対象面が関係する Security Principles の各境界を検証し、続いて `security-checks.md` の該当する Stable Pattern と Trend-Sensitive Pattern を実装スコープに対して実行する。

変更が依存関係、ランタイム、パイプラインアクションのバージョンやリビジョンを変える場合、または正典が最新アドバイザリの確認を求めている場合は、その対象コンポーネントとバージョンについて権威ある最新アドバイザリを確認する。変更がこのプロジェクトで初めて使う依存関係やパイプラインアクションを導入する場合、またはその取得元を変える場合は、名前が想定する公開者またはソースリポジトリに解決されることも確認する。新しく採用したコンポーネントは、アドバイザリが出る前に攻撃者が登録したものだったり、名前を変えられたものだったりする可能性があるためである。それ以外の場合、アドバイザリの確認は行わない。
...
- アドバイザリの確認は、変更または新たに採用した依存関係・ランタイム・パイプラインアクションごとに行い、それ以外では行っていない
