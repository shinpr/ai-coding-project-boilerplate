...
5. **タスクの構造化**
   ...
   - `## Proof Obligations`（クレームごと — 下記「証明義務の伝播」参照）
   - `## Completion Criteria`
...
クレームを実装する各タスクは Proof Obligations（task template参照）を持ち、下流のレビューがテストがクレームを証明しているか（単に実行されるだけでないか）を判定できるようにする:

1. **出所**: テストスケルトンがタスクをカバーしている場合、その「主要な故障モード」と「証明義務」の注釈をタスクの Proof Obligations に転記する。スケルトンがクレームをカバーしていない場合、主要な故障モードをACから導出し、境界・操作前後の状態アサーション・モック境界の根拠・残余をACとタスクの対象ファイルから導出する（そのクレームに該当しないフィールドは `N/A` とする — 例: 状態を変更しないクレームでは状態アサーションなし）。
2. **クレームごと**: ACまたはクレームごとに1エントリを記録し、task template で定義された Proof Obligations の全フィールドを埋める。
3. **クレームがある場合に適用**: 振る舞いのクレームを持たないタスク（純粋な設定やスキャフォールディング等）は本セクションを省略する。
...
- [ ] クレームを実装する各タスクに Proof Obligations を記録（主要な故障モード + 検証する境界）
