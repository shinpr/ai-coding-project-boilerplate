...
各ACの主要な故障モードと証明義務は、テストのスケルトン注釈（「主要な故障モード」/「証明義務」コメント）を出所とする — これらは task template の Proof Obligations フィールドに対応する。各テストがクレームを証明していることを確認する: アサーションが約束された振る舞いを観測し、その振る舞いがリグレッションするとテストが失敗する。テストが未証明のまま残す各義務について `proof_insufficient` を記録する:
...
- [ ] すべてのスケルトンコメントを実装と照合
- [ ] 実装品質を評価
- [ ] 各テストがACのクレームを証明している: 主要な故障モードでレッドになり、主張された境界を通過し、状態変更を伴うクレームでは操作前後の状態をアサートする
- [ ] Mock境界を検証（統合テスト）
