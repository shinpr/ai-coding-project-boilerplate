...
2. **BP-002**: 曖昧な表現を、必要な精度を満たす最も制約の少ない基準（除外する有効な挙動が最も少ない、測定可能なルール/閾値）で解決する。適用した明確化については精度への寄与・制約コスト・導出元の情報源を記録する。入力やプロジェクトのコンテキストから判断できない場合は、推測せず判断に必要な情報源を記録する
...
7. **BP-007**: 例は必要最小限にとどめる — 一般に知られた挙動には簡潔なルールまたは利用側が要求する出力形式を使い、ドメイン・製品・組織固有のマッピング、非自明な例外、ルールで表現できない境界を覆う最小限の例集合のみ追加する
8. **BP-008**: 主張を観測事実・推論・不明に分類する。エスカレーション基準を追加し、「不明」が次のステップを塞ぐ場合は、続行に必要なエビデンスまたはユーザー判断を明示する停止条件を設ける
9. **BP-009**: 成果、境界、実際の利用側、必要な証明に不要な作業を除き、変更なし、再利用、根拠に基づく却下を有効な結論として残す
...
分割前に`skill-optimization/references/creation-guide.md`の圧縮手順を適用する。圧縮後も400行を超える場合は、読み込み条件と判断への影響が明示された条件付き詳細だけを抽出する。250行を超える本文は、セクションを削除するとベースライン外の判断、必要な境界、利用側の契約、または観測済みの反復障害への対応が変わる場合に限り保持する。
...
- **`reviewResolutions`**: 現状レビューがある場合だけ出力する。reviewerの各指摘を`findingId`で裁定し、`decline`には根拠を必須とする
