...
UI Specが存在する場合、以下のプロセスでDesign Docに統合:

1. UI Specのコンポーネント構造と状態設計を確認
2. 状態×表示マトリクスからデータモデルとAPIコントラクトを導出
3. インタラクション定義からエラーハンドリングパターンを特定
4. UI Specの受入条件をDesign Docのテスト戦略に反映
5. 既存コンポーネント再利用マップをDesign Docの実装計画に統合
