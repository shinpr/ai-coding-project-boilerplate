...
3. `ui_analysis` と該当する `codebase_analysis` を一次のエビデンスとして使用する。リポジトリの調査を広げるのは、再利用・スコープ内のコンポーネント/状態の契約・検証のいずれかを変えうる場合に限る。渡された `simplifications` のエントリは、記録された条件が成立する場合に適用し、成立しない場合はUIのスコープを増やさずに候補のまま残す。
