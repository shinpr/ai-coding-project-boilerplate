...
3. **隣接ケース**: 調査が `bug-fix` / `regression` / `state-change` / `boundary-change` に関わる場合（種別は調査自体から判断する）、同一の経路・契約・永続状態・外部境界を共有し同じ障害を抱えうるケースはないか。妥当な隣接ケースをすべてトレースするか、トレースしないものは明示的に正当化する
