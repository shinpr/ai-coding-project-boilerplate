...
- Selected Design を左右する前提が、承認前に現在のエビデンスで解決されている。Risks に残るのは、どの結果でも Selected Design が有効なままである実装上の残余不確実性だけであり、必要な場合はスコープ内で実行可能な検証またはガードが記録されている。
- 提供されたコード/UIの各 focus area に、エビデンスを保持した Fact Disposition の行が1つある。
- Selected Design が成果を届けている。追加された各設計サーフェスが、現行要件・検証済みの制約・観測された問題・受理済みの決定・根拠のある重大リスクのいずれかを解消し、より小さいサーフェスでは不十分な理由と減算のエビデンスによって、追加した複雑性の必要性が示されている。
...
- 現時点のエビデンスで総合的な複雑性を正当化できず、取り除いても確認済みの成果・境界・必要な証明が成立する設計サーフェスを、下流の実装へ確定事項として渡している
