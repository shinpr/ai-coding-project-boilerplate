...
3. 候補となる振る舞いをMVPに残すのは、それを取り除くと明記された価値、または必要な法務・契約・セキュリティ・互換性の義務が破れる場合に限る。残す根拠となった除去結果を記録する
4. `Out of Scope` に記録するのは、ユーザーが明示した除外事項と、MVPの境界を実装判断に使える形で定めるために必要な現在の除外に限る
...
**ユーザージャーニー図**・**スコープ境界図**は、散文では重要なフローや境界が明確にならない場合に限り、いずれか一方または両方を使用する。追加の図表も、別の重要な関係性を解消する場合にのみ使用する。
