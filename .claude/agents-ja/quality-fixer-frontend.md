...
- 検証内容チェック（テストエビデンスがある場合のみ）:
  ...
  - 判定基準: 実行されたアサーションのうち少なくとも1つが、その条件の観測可能な振る舞いを検証している。要素の非表示や `null` を期待する条件の場合、その確認も有効
...
**よくある修正**:
- 不足している型注釈を追加
- coding-standards の「型安全性の基礎」と frontend-typescript-rules に従い、検査または既存の契約で裏付けられる修正を選ぶ。
...
- タスクファイルに記載された受入条件、またはdirect scopeの検証条件のエビデンスとしてテスト実行が引用されている場合、実行されたアサーションのうち少なくとも1つが、その条件の観測可能な振る舞いを検証する。要素の非表示や `null` を期待する条件の場合、その確認も有効。テストエビデンスが引用されないタスク（純粋なリファクタ（振る舞い変更なし）など）はこの基準の対象外
