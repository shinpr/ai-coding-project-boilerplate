...
- **qualityCommand**（任意）: 呼び出し側が把握している場合の、プロジェクトで正となる品質コマンド（例: frontend-technical-spec やリポジトリの規約由来）。指定された場合、ステップ2はまずこれを実行し、これがカバーしないカテゴリについてのみコマンドを検出する。指定がない場合、ステップ2は従来どおりプロジェクト設定からコマンドを検出する。
...
- 検証内容チェック（テストエビデンスがある場合のみ）:
  ...
  - 判定基準: 実行されたアサーションのうち少なくとも1つが、AC の観測可能な振る舞いを検証している。AC が要素の非表示や `null` を求める場合、その確認も有効
...
- 振る舞いの証明不足:
  - ユーザー可視要素には role/name クエリを優先。非同期な出現には `findBy*`/`waitFor`、非表示の確認には `queryBy*`/`queryAllBy*` を使う
...
- タスクファイルに記載された AC のエビデンスとしてテスト実行が引用されている場合、実行されたアサーションのうち少なくとも1つが、その AC の観測可能な振る舞いを検証する。AC が要素の非表示や `null` を求める場合、その確認も有効。テストエビデンスが引用されないタスク（純粋なリファクタ（振る舞い変更なし）など）はこの基準の対象外
...
- テスト修正判断、RTL/MSW 規約、検証内容の基準: frontend-typescript-testing スキル
...
- **明確なコード品質問題**
  - 未使用の変数・関数・コンポーネント削除
  - 今回の変更によって不要になったexportは、利用側を確認してから削除する。別の責務に属する未使用と思われるexportは、その問題のエビデンスとして記録し、この修正では削除しない
  - 到達不可能コードの削除
  - console.log文の削除
...
- **React Testing Libraryテスト修正**: プロジェクトテストルールの判断基準に従う
  ...
  - 境界値テスト失敗: 仕様を確認して修正
- **バンドルとレンダリングの修正**（エビデンスがある場合のみ。判断規則は frontend-typescript-rules を参照）
  - 設定済みのバンドル予算にリグレッションがある場合、またはリポジトリのバンドル分析・プロファイラが今回の変更箇所を原因として示す場合に、メモ化またはコード分割を行う。変更前後で同じ指標を再確認し、両方の結果を記録する
  - その指標がない場合は、観測結果を別途フォローアップ用のエビデンスとして記録するにとどめる。パフォーマンス変更には、計測されたリグレッションまたは受け入れ済み要件が必要である
- **アクセシビリティ修正**
  - ARIAラベルとロールを追加
  - 色のコントラスト問題を修正
  - 画像にaltテキストを追加
