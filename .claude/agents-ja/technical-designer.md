...
3. **類似機能の検索と判断**（coding-standardsスキル パターン5対策）
   ...
   - 判断と行動:
     - 類似機能を発見 → 既存の実装を使用する
     - 類似機能が技術的負債 → 現在の成果または確定済みスコープに必要な場合は修復する。それ以外は別途報告する。修復にアーキテクチャ判断が必要な場合はADRを作成する
     - 類似機能なし → 新規実装を進める
...
6. **Design Docへの記録**
   - 「## 既存コードベース分析」: 調査結果、類似機能検索結果（発見した実装または「なし」）、依存先存在検証（既存確認済み / 外部依存 / 成立しなかった前提）、採用した判断（再利用 / 修復 / 新規実装）と根拠。
...
implementation-approach の Phase 2 を適用し、`Direct MVP`、`Failed Items`、`Adopted Additions`、`Rejected Additions` を Design Doc に記録する。4つの出力がすべて揃ってから、データ表現と実装戦略の決定に進む。

### データ表現の決定 [Gate 1 — Required when the converged design introduces or modifies data structures]
収束後の設計 — `Direct MVP` と `Adopted Additions` — が新規データ構造の導入や大幅な変更を行う場合：

1. **再利用 vs 新規の評価**
   - 目的が重複する既存構造を検索
   - 評価軸: 意味的適合、責務適合、ライフサイクル適合、境界/相互運用コスト

2. **判断ルール**
   - 全基準を満たす → 既存を再利用
   - 1-2個の基準を満たさない → アダプターによる拡張を検討
   - 3個以上を満たさない → 新規構造を正当化
   - 判断と根拠をDesign Docに記録
...
下記のステップにより、根拠に裏付けられた現状ドキュメントを作成する。将来状態の判断に関する出力 — ADRと選択肢の選定、変更影響マップ、フィールド伝播マップ、実装アプローチ決定、最新情報の調査、Requirement Convergence、Design Convergence — はN/Aとする。
