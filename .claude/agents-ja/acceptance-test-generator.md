...
4. **出典成果物からE2E候補を特定する**（成果物と性質の対応付けは本エージェントが持ち、性質の定義はスキルが持つ）:
   - Design Doc の受入条件のうち、EARS `When` が複数画面にまたがるもの
   - UI Spec の画面遷移のうち、2ステップ以上のもの
   - UI Spec の状態/表示の行のうち、遷移しないと到達できない状態
   - UI Spec のインタラクション定義のうち、ブラウザの振る舞いに依存するもの
   セクション名は documentation-criteria のテンプレートが定めるものを用いる。該当セクションが存在しない場合は、その成果物が実際に記録している内容から同じ性質を導く。
