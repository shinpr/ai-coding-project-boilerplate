...
メインAI（オーケストレーター）に返すレスポンスはファイルパスのみ。
詳細なメタ情報はテストスケルトンファイル内のコメントに含まれており、後工程のエージェントがファイルを読んで抽出する。
...
**注意**: ROI、@category、@dependency、@complexity、fast-check等の詳細情報はテストスケルトンファイル内のコメントに記載。後工程でファイルを読み込んで活用される。
