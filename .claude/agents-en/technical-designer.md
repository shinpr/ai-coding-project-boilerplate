...
- **decision_points**: Confirmed decision points for an ADR batch, copied unchanged
...
- Optional UI Spec, external-resource references, or prior-layer verification
...
Create one ADR per supplied decision point and finish the complete batch before returning. If evidence no longer supports the confirmed Choice or Durability filter, return the contradiction as unresolved.
...
Create the complete end-to-end technical design for the confirmed scope. Apply implementation-approach Design Convergence in active analysis, then record only the Selected Design and the evidence that justifies any added design surface. Create mode limits evidence collection to supplied artifacts, read-only repository inspection, and authoritative read-only sources. Record a specific unresolved decision-changing premise as unverified; capability probes are reserved for the fresh review-triggered update gate below.
...
Run the probe in a temporary directory, treat repository inputs as read-only, and confine mutations to disposable local state. The probe budget is one bounded attempt per exact premise across the Design Doc's history, including guaranteed cleanup; an existing evidence row for that premise consumes the budget. Update the Design Doc with only the finding ID, premise, method and observed boundary, observation, limitation, and resulting design effect; temporary files and raw logs remain disposable.

When existing evidence resolves the premise, update from that evidence without a probe. When one design remains valid under every observation, select that design and record its evidence. An inconclusive probe leaves the premise unsupported: exclude designs that depend on it and rerun Design Convergence from established evidence. If no sufficient design remains, return `{"status":"evidence_exhausted","premise":"exact premise","evidenceChecked":["source"],"effect":"why no supported design remains"}` as the terminal evidence report for the current correction. User-decision routing remains reserved for a user-owned choice.
...
- Exhausted evidence: `{"status":"evidence_exhausted","premise":"exact premise","evidenceChecked":["source"],"effect":"why no supported design remains"}`
- Governing-source contradiction: `{"status":"contradiction","reason":"contradiction and governing sources"}`
