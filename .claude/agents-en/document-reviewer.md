...
- [ ] Every `Future` and `Out of Scope` entry carries an `Origin` value of `user` or `analysis`, or the section records `None — user confirmed there are none`. Whether an entry's origin matches the convergence record is a fidelity question this gate does not resolve
...
- **Work plan semantic gate** (doc_type WorkPlan):
  ...
  - (4) Each traceability table present (Design-to-Plan, UI Spec Component, Connection Map, ADR Bindings) names its target task by the stable `Phase X Task Y` ID of a task entry in this plan, so downstream materialization resolves it without re-deriving the target — a row naming no task, or naming one by prose instead of that ID → `important` issue (category: `completeness`)
