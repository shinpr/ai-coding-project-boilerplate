...
For PRD at `review_context: creation`, additionally verify:
- [ ] Every `Future` and `Out of Scope` entry carries an `Origin` value of `user` or `analysis`, or the section records `None — user confirmed there are none`. Whether an entry's origin matches the convergence record is a fidelity question this gate does not resolve. At `review_context: update` or `as-is`, entries predating the Origin marker are out of scope; check only entries the revision added
...
- **Work plan semantic gate** (doc_type WorkPlan):
  ...
  - (4) Each traceability table present (Design-to-Plan, UI Spec Component, Connection Map, ADR Bindings) names its target task by the stable `Phase X Task Y` ID of a task entry in this plan, so downstream materialization resolves it without re-deriving the target — a row naming a task by prose instead of that ID → `important` issue (category: `completeness`). Rows whose Gap Status is `gap` are covered by check (1) and are out of scope here
  ...
  - (5a) When `irreversible-operation` is marked yes, the First-Pass Risk Coverage table carries a row per irreversible operation with its reaching routes, its safe default on incomplete evidence, a `Covered By Task(s)` value naming one or more stable `Phase X Task Y` IDs, and a disposition in every one of the six hazard columns (mutation, partial-evidence, retry, concurrency, identity, input-route) — a missing table, a blank hazard cell, a `Covered By Task(s)` value that names no such ID, or a `blocked` hazard with no matching entry in Decisions and Unresolved Items → `important` issue (category: `completeness`). A blank cell is the defect to catch here: task materialization copies these rows onward by matching that ID, so a disposition missing at this stage never reaches an implementer
...
- Only `important` or `recommended` issues
- Only easily fixable issues
- Prior context items (if any): At most 1 `important` unresolved
...
- Prior context items (if any): 2+ `important` unresolved OR any `critical` unresolved
