...
完了レポートの前に、レビュー由来の修正や証明不足の再試行が最後のタスクコミット以降に未コミットで残した変更を、該当するquality-fixerが `pass` または `verification_incomplete` を返した後にコミットする。その後、本レシピが処理した実装タスクファイルを削除する。これで作業内容はコミット済みとなり、`docs/plans/`はレシピ実行間で保持しない一時的な作業状態である:
