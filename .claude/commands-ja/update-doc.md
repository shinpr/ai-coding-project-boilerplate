...
**更新へ引き継ぐスコープ**: 確認済みのセクション、変更の理由、およびユーザーが述べたサイズ予算を更新エージェントに渡す。ドキュメントレビューの前に、変更された各セクションを、確認済みの変更、またはその変更が必要とした整合性の更新に対応付ける。意図せず対応付けられない変更は取り除く。必要な変更によって依頼されたドキュメントの成果または明示されたサイズ制約が変わる場合は、依頼内容の確認ゲートへ戻る。
...
- `rejected` → 確認済みの成果、将来状態の要件、対象外を同時には維持できない場合は上位の要件ゲートを適用する。それ以外の技術上の衝突はレビュー裁定で解消する。
...
**整合性結果に基づく対応**:
- 矛盾なし → 最終承認の要約に結果を含める
- 矛盾を検出 → design-syncを新しいverifierとしてレビュー裁定を適用する。`apply`の矛盾はステップ4の担当ドキュメントへ戻し、修正後にdesign-syncを再実行し、エビデンスに基づく却下は完了として維持する。
...
| レビュアーが `rejected` を返す | 確認済みの成果、将来状態の要件、対象外を同時には維持できない場合は上位の要件ゲートを適用し、それ以外の技術上の衝突はレビュー裁定で解消 |
| design-syncが矛盾を検出 | レビュー裁定を適用し、適用対象の矛盾を担当する更新経路へ戻す |
