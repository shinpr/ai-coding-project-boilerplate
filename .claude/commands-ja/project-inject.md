...
Step 1-6を順番に完了する。現在のステップで定められたチェックポイントまたはユーザー判断を満たした場合にのみ次へ進む。検証ステップが成功した後に結果を提示する。

### Step 1: 入力読み込み

1. `.claude/skills/project-context/SKILL.md` を読み、現在の状態を把握する。本文に含まれるカタログセクション見出し（`## プロジェクト概要`、`## ドメイン制約`、`## 品質基準`、`## 開発フェーズ`、`## ディレクトリ規約`、`## 外部リソース`）の数で 2 つのケースを区別する:
...
**曖昧ルールの差し戻し**（`add` および `update` 区分の全セクションに適用）: ユーザー提供のルールが主観的な表現の場合（例: 「パフォーマンスに気をつける」）、フォローアップで尋ねる: 「AI はこのルールが pass か fail かをどのように検証できるか？ 測定可能なチェックに書き換える、または 'drop' と返答すれば省略する。」 測定可能な形で提示されたルールはそのまま採用し、主観的なものはユーザーが書き換えた版で置き換え、'drop' の場合はそのルールを省略する。
...
1. フロントマター — 以下の正典ブロックをそのまま書き込む:
   ```
   ---
   name: project-context
   description: AIの実行精度に必要なプロジェクト固有の前提情報を提供 — ドメイン制約、リポジトリ固有の品質基準、開発フェーズ、ディレクトリ規約、外部リソースのアクセス方法。セッション開始時、プロジェクト構成確認時、またはタスクがドメインルール、リポジトリ固有の品質基準、リポジトリ外の外部リソースのいずれかを参照する時に使用。
   ---
   ```
2. `# プロジェクトコンテキスト` 見出し。
3. 各セクションをカタログ順（プロジェクト概要 → ドメイン制約 → 品質基準 → 開発フェーズ → ディレクトリ規約 → 外部リソース）で配置する。出力に含まれるのは区分が `add`、`keep`、`update` のいずれかであり、かつ採用ルールを満たすセクションのみ。採用したセクションを順に並べ、間に他の記述は挟まない。

組み立てた内容を `.claude/skills/project-context/SKILL.md` に書き込む。このパスは、Claude が毎セッション読み込むランタイムの正典である。

### Step 5: 検証

再構築した `.claude/skills/project-context/SKILL.md` に対して以下のチェックを順に適用する:

- [ ] フロントマターが Step 4 の正典ブロックと逐語一致する。
...
- [ ] 各品質基準が、観測可能な適用条件、1つの観測可能な肯定形の合格状態、根拠となるエビデンスを備えている。
- [ ] プロジェクトコンテキストの内容はリポジトリ固有の制約・品質基準・フェーズ・規約・外部リソースに限られる。技術スタックの詳細は `technical-spec` スキルの責務、一般的な実装原則は `coding-standards` スキルの責務である。
...
- 追加されたセクション: プロジェクト概要、ドメイン制約（2 ルール収集）、品質基準（2 件収集）、開発フェーズ、ディレクトリ規約（1 ルール）、外部リソース（バックエンドドメイン — データベーススキーマソース、シークレットストア）
- 削除されたセクション: （なし）
