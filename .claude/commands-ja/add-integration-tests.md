...
executorの呼び出しごとに「専門エージェントの結果の受理」を適用する。応答とリポジトリの状態から、変更された統合テスト/E2Eテストが1つ以上確認できたらStep 4へ進む。進められる作業が残っている間は実装を続ける。
...
```
サブエージェントのスコープ境界:
受け入れ済みのテストによる証明を、リポジトリ内の担当範囲全体で一貫して完成させる。
