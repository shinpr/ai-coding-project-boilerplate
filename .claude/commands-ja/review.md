...
ユーザーに尋ねるのは2つだけとする: 提案した `apply` 集合を適用する権限と、`user_decision_required` の各項目に対する判断。`user_decision_required` の項目でユーザーが「コードが正しく Design Doc が陳腐化している」と判断した場合、その項目はステップ5へ回す。承認された変更が残らない場合はステップ10へ進む。

**修正パスへ引き継ぐスコープ**: 承認された検出事項、対象となるファイルとセクション、およびユーザーが述べたサイズ予算を、ステップ5〜9 で呼び出す全エージェントに渡す。再検証の前に、各差分ハンクを承認された検出事項、またはその検出事項が必要とした整合性の更新に対応付ける。対応付かないハンク、または述べられた予算を超える差分については、修正の一部として受け入れるのではなくスコープ判断を求める。
...
4. 承認済みの `apply` 検出事項を更新後の Design Doc に対して再評価し、改訂で既に満たされたものは除外する。残りがない場合はステップ6〜7をスキップしてステップ8へ進む。
