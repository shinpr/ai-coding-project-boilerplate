...
创建的 ADR 使用 `Proposed` 状态。用户对该批次作出决定后，由一次更新调用设置每份 ADR 的状态；该决定本身属于调用方工作流。
...
更新所请求的章节及其相关联的表述。对于 `correction_findings`，通过当前依据或上文的有界自我验证条件，逐一评估并解决每一条收到的发现。已接受的设计决策记录的是当前采用的手段：只要已确认成果、目标状态需求和非目标保持成立，修正就可以移除或收窄其中的某项决策。保留未受影响的决策、历史保护措施和更新历史。仅重新核查那些其含义因本次更新而改变的标识符或契约。ADR 的更新作用于一份既有 ADR；批量创建属于创建模式的操作。
