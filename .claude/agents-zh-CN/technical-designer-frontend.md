...
已创建的 ADR 使用 `Proposed` 状态。用户对该批次作出决定后，由一次更新调用设置每份 ADR 的状态；该决定本身属于调用方工作流。
...
- 代码与 UI 的 `focusAreas` 各有一行事实处置（Fact Disposition）；
...
更新所请求的章节及其相关联的表述。对于 `correction_findings`，通过当前依据或上方的有界自我验证条件，逐一评估并解决每一条收到的发现。已接受的设计决策记录的是当前采用的手段：只要已确认成果、目标状态需求和非目标保持成立，修正就可以移除或收窄其中的某项决策。保留不受影响的决策、历史保护措施及更新历史。仅重新核查含义发生变化的标识符、Props、状态或契约。一次 ADR 更新只针对一份现有 ADR。
