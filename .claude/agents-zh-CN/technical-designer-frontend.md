...
更新所请求的章节及其相关联的表述。对于 `correction_findings`，需在已确认需求和现有职责的范围内，通过当前依据或上方的有界自我验证条件，逐一评估并解决每一条收到的发现。已接受的设计决策记录的是当前采用的手段：只要已确认成果、目标状态需求和非目标保持成立，就可以移除或收窄其中的某项决策。若某条发现不改变已确认需求或现有职责就无法解决，则返回 `contradiction` 结果，并附上不成立的条件及依据。保留不受影响的决策、历史保护措施及更新历史。仅重新核查含义发生变化的标识符、Props、状态或契约。一次 ADR 更新只针对一份现有 ADR。
