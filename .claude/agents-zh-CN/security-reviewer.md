...
每条发现项包含一个必须修复的问题及其最小充分修正方案。可选的加固措施和纵深防御不出现在响应中；当只存在这类候选项时，返回 `pass`。
...
对于破坏性操作、持久状态变更，或能到达变更操作的边界变化，识别该操作、其可达路径，以及依据不完整时的安全行为。当变更授权、重试、并发、身份和输入路径的处理可能影响该操作的安全性时，对其进行检查。
...
验证变更后的攻击面所涉及的每一条安全原则边界，然后针对实现范围执行 `security-checks.md` 中适用的稳定模式和趋势敏感检测模式。

当变更改动了依赖、运行时或流水线 action 的版本或修订，或约束文档要求验证最新安全公告时，针对该确切组件及版本查阅权威的最新安全公告。当变更引入了本项目此前未使用的依赖或流水线 action，或改变了其解析来源时，还要确认该名称解析到预期的发布者或源仓库；新采用的组件可能在出现任何安全公告之前就已被攻击者抢注或改名。其他情况下跳过安全公告检查。
...
- 已针对每个被改动或新采用的依赖、运行时或流水线 action 执行安全公告检查，其他情况下已跳过
