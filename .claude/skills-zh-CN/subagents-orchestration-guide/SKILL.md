...
- **首个行动**：在任何其他步骤之前，按照首个行动规则的交接方式调用 requirement-analyzer

### 首个行动规则

在收到新任务时，将用户的完整原话保留在编排者手中，只向 requirement-analyzer 提供调查目标：`requirements` 为用户对问题或用户可见、运营层面成果的最简短原话，仅当不存在这样的原话时才使用编排者的工作摘要；`context` 为解读该成果所需的最简短用户理由、仅在必不可少时附上的环境约束，以及用户点名的路径、标识符或产物，或改变了分析对象的询问答复。编排者选定的路径和技术性问题不放入交接内容。将返回的范围、成本和问题依据与所保留的原话进行对照，以执行需求收敛并分配结构规模。评估性请求、设想性想法和指定的机制要根据这些原话来分类；若由分析器分类，推断就会与仓库观察结果混在一起。产品需求和排除项归用户所有；收敛就绪度和结构规模由编排者负责。仅当询问所得的答复改变了分析对象或所需的范围依据时，才重新调用 requirement-analyzer。

### 流程中的需求变更检测

分析结果返回后，将其揭示的每项用户可见或运营层面的职责与已确认范围进行对照。当保持该职责不变或对其进行变更会改变已确认成果或某项排除项时，在进行依赖它的设计工作之前，将其送回需求确认关口。至于如何满足已确认范围的选择，仍由设计负责方决定。
...
评审者的发现是候选项。只从评审裁定的 `apply` 集合中创建修正工作。评审者的 `pass` 只记录阶段已通过；它不授予任何由用户持有的权限，也不能证明被评审的手段仍然必要。
...
**重新运行规则**：一旦某个评审者返回过通过（passing）的结果，就不再重新运行它。只重新运行其最新结果中已应用修正的那个评审者，并将已记录的处置作为 `prior_feedback`、连同重新推导的实现文件集合一起传入，使其针对修正后的状态进行复核。恢复被阻塞的评审前置条件后，重新运行相应评审者。是否接受修正由评审裁定的收敛机制决定，已解决的拒绝项保持有效。
