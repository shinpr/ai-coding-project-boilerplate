...
| 步骤 | 动作 | 完成依据 |
|------|--------|---------------------|
| 1 | 按下方格式呈现范围确认，只就低于 `ready` 的字段提问 | 每条事实都注明其来自哪份分析输出，且**待用户决定事项**中每个低于 `ready` 的字段各有一个问题 |
| 2 | 将每个答案记录为该字段的值 | 该值是用户明确选定的选项，或用户自己提供的措辞 |
| 3 | 当已记录的值仍未通过其通过条件时，重新询问一次；若用户同意将第二次回答维持原状，则将该字段标记为 `weak-but-explicit` | 两次已记录的答案，或用户同意停止 |
| 4 | 依据每个字段的通过条件进行判断，并完成该记录 | 一份每个字段都已标注的收敛记录 |

## 范围确认

在每个需求确认停止点都按此格式呈现，无论是否进行了询问；只纳入可能改变用户需求决定的内容。

| 部分 | 内容 |
|------|------|
| **已确认范围** | 用户已经陈述或明确选定的需求和排除项，使用用户自己的措辞或所选的选项 |
| **判断依据** | 每个实质性条目都标明是观察所得、推断还是提议，并附上其来源以及它可能对范围、成果或成本带来的改变，其中包括成本分级及其仍然存在的未知项 |
| **待用户决定事项** | 每个尚未解决的产品、UX 或运营问题，以及其实质上不同的答案分别对范围、成果或成本产生的影响 |

将这几部分分开呈现，用户才能分辨哪些是自己已定下的边界、哪些是仓库观察结果、哪些仍是待决的问题。只有用户的明确答复或选择才能将某项移入**已确认范围**。调用方工作流自己的路由事项（如结构规模）放在这几部分之外，且只在会呈现它们的停止点呈现。
...
- [ ] 在提出任何问题之前，范围确认已将已确认范围、判断依据和待用户决定事项分开呈现
