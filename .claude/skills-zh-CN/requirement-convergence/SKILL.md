...
编排者同时负责引出信息和做出判断。它使用分析者提供的范围与成本依据，然后只就仓库无法回答的产品选择进行提问。仅当某个答案会改变分析目标或所需的范围依据时，才重新运行分析者。

在开始前登记以下步骤，并在每个步骤完成时记录其依据：

| 步骤 | 动作 | 完成依据 |
|------|--------|---------------------|
| 1 | 陈述分析产出的范围事实，然后单独说明这些事实对需求意味着什么 | 列出事实及其来源的分析输出 |
...
| 5 | 依据每个字段的通过条件进行判断，并将完成的记录传递给下游 | 一份每个字段都已标注的收敛记录 |

## 存储协议

| 载体 | 承载内容 | 撰写者 |
|---------|-------|------------|
| 编排者的收敛记录 | 每个字段及其就绪标签 | 编排者 |
| PRD 的 `成功标准` 和 `范围外` | `outcome`；用户撰写的 `nonGoals` | 负责该 PRD 的智能体 |
| 设计文档的 `需求收敛` | 在没有 PRD 时承载同样内容，且在任何情况下都承载被标为 `weak-but-explicit` 的字段 | 负责该设计文档的智能体 |

不产生上述任一文档的流程，将记录随其自身上下文带入下一步骤。

## 引用协议（供下游使用方使用）

1. 从提示词中读取收敛记录。
2. 将 `nonGoals` 视为当前变更排除在外的内容，将 `desired-future` 需求视为可构建的范围。未被采纳的评估性请求、设想性想法和指定的实现机制不产生任何下游义务；已被采纳的 ADR 可以将已评估过的选项保留作为决策历史。
