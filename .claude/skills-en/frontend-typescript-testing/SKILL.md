...
3. **Cross-functional Verification**
   - When a new feature reaches a shared integration point, continuity of the existing feature becomes a proof obligation if its failure would break a primary user journey or contract, or degrade a secondary observable behavior. Select the boundary that proves it per integration-e2e-testing
