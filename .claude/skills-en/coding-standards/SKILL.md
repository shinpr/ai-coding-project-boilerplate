...
- **Design Convergence** - Deliver the current required outcome with the least new design surface. Selecting persistent state, public or cross-boundary contracts, behavioral modes, reusable abstractions, or component splits carries enough surface to run the implementation-approach skill's Phase 2 (Design Convergence) first
