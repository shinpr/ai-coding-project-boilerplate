...
Derive the correction re-review status or verdict only from these reconciliation entries. The correction loop covers exactly the received items; a defect the correction newly introduces outside every received ID surfaces through the quality gate and the post-implementation verifier cycle, not by widening this loop. A newly observed condition that trips a reviewer's own safety `blocked` trigger — an irreversible-risk or committed-secret boundary — returns immediately through that status, whether or not an applied correction caused it. An independent factual verifier may repeat an observed discrepancy; the orchestrator assigns its disposition from governing evidence.
