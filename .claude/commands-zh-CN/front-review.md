---
description: 评审已完成的前端实现，检查其与约束来源的一致性、变更范围的必要性、仓库质量与安全性，然后应用用户已批准的 React 修正。
---
...
**命令上下文**：面向 React/TypeScript 前端的实现后质量保障命令

## 编排者定义

**核心身份**：“我是编排者。”（参见 subagents-orchestration-guide 技能）

**执行条件**：按顺序完成第 1-11 步，仅沿其所述条件所激活的分支执行。每一次评审、修正与重新验证的转换，仅在其声明的收敛条件处推进。在每一项适用的发现项都达到其所需处置或重试结果之后，再呈现最终报告。
...
### 步骤 1：前置条件检查
首先从 `$ARGUMENTS` 解析设计文档。若未指定，则从仓库元数据、引用和内容中查找约束已变更前端职责的文档。从其上游分支与仓库默认分支解析分支比较基线，然后列出从该合并基线到 `HEAD` 的实现文件。

### 步骤 2：执行 code-reviewer
使用 Agent 工具调用 code-reviewer：
- `subagent_type`: "code-reviewer"
- `description`: "已完成的前端实现评审"
- `prompt`: "评审已完成的前端实现。governingDocuments: [{\"type\":\"design-doc\",\"path\":\"[path]\"}]。implementationFiles: [git diff file list]。返回首次评审的 JSON。"

**将输出保存为**：`$STEP_2_OUTPUT`

### 步骤 3：执行 security-reviewer
...
对两份输出应用“评审裁定”。其 `apply` 与 `decline` 处置决定路由。按“评审裁定”第 2 节为每一项 `apply` 发现项选择修正责任方：技术产物的变更走设计侧，实现的变更走代码侧；若要移除某个由设计文档选定、而已确认成果并不需要的机制，则先走设计侧、再走代码侧。
...
- `description`: "执行评审修复"
- `direct_scope`: 在已确认的评审范围与所述的总规模预算内应用已批准的前端修正
- `governing_sources`: 被评审的设计文档、适用的 UI 规范，以及已接受的需求或 ADR 路径
- `target_paths`: 为已批准的代码侧路由所确认的实现与测试路径
- `observable_verification`: 由发现项与约束来源所指明的聚焦 UI 行为测试或可观测契约检查通过
...
使用 Agent 工具调用 code-reviewer：
- `subagent_type`: "code-reviewer"
- `description`: "重新验证前端实现评审"
- `prompt`: "在已批准的修正之后，重新评审已完成的前端实现。governingDocuments: [{\"type\":\"design-doc\",\"path\":\"[path]\"}]。implementationFiles: [file list]。prior_feedback: [{id, disposition, reason?, evidence}]。核对收到的每一项。"
...
**范围**：已完成的前端实现评审、安全性评审，以及用户已批准的修正路由。
