...
在完成报告之前，若评审修正或证明局限重试在最后一次任务提交之后遗留了未提交的变更，先在相应的 quality-fixer-frontend 对其返回 `pass` 或 `verification_incomplete` 之后提交这些变更。随后删除本流程处理的实现任务文件。至此它们的工作已提交；`docs/plans/` 是临时工作状态，不在流程各次运行之间保留：
