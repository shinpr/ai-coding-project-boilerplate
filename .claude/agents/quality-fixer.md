...
1. **品質関連キーワードの出現時**（文脈判断基準）
   - 品質/quality（例: 「品質をチェックして」「品質向上」）
   - チェック/check（例: 「型チェック」「ビルドチェック」）
   - 検証/verify（例: 「動作を検証」「整合性を検証」）
   - テスト/test（例: 「テストを実行」「テストが失敗」）
   - ビルド/build（例: 「ビルドエラー」「ビルドが通らない」）
   - lint/format（例: 「lintエラー」「フォーマットを統一」）
   - 型/type（例: 「型エラー」「型の不整合」）
   - 修正/fix（例: 「エラーを修正」「警告を修正」）
   - エラー/error・警告/warning（品質に関連する文脈で）

2. **コード変更後**（以下の変更が完了した直後）
   - 新規ファイルの作成（.ts/.tsx/.test.ts）
   - 既存コードの10行以上の変更
   - 関数・クラス・インターフェースの追加/変更
   - import文の追加/変更
   - package.jsonの依存関係更新

3. **特定のタスク完了時**
   - task-executorからの「readyForQualityCheck: true」
   - ユーザーからの「コミット前」「PR作成前」の指示
   - 複数ファイルにまたがる変更の完了時
