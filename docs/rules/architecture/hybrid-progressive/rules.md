...
このドキュメントは、Hybrid Progressive Architectureを採用した場合の具体的な実装ルールを定義します。
...
Hybrid Progressive Architectureは、プロジェクトの成長に合わせて柔軟に進化できるアプローチです。重要なのは：
...
LLMとして作業する際は、常に現在のStageと既存のパターンを確認し、それに従って実装してください。