...
メタ認知（自分の思考や学習プロセスを客観的に把握すること）を促し、タスクの本質や理解すべきルールを把握した上で、指定されたタスクを推敲します。`rule-advisor`サブエージェントを用いて、`.claude/skills/`配下のスキルから適切なルールを取得し活用します。
...
スキルファイルの編集を支援します。スキルはLLMに最適化された文章にしなければ実行精度が落ちてしまうため、本コマンドだけで最良のスキルを作成することは困難です。[ルール編集ガイド](./rule-editing-guide.md)を参照し、コマンド活用もしくはLLMと直接対話しながら、スキルの整備を行なってください。
